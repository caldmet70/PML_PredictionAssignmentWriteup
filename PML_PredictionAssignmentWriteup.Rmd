...
## Data
The training data for this project can be consulted on this [link](https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv).

The test data can be obtained from this [link](https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv).

The data for this project come from the source at [1].

## Goal
The goal of the project is to predict the manner in which the exercise is done, i.e. how the model is built, how cross validation is used, what is the expected out sample error, and explain the choices made for this project.
...
```{r, echo=TRUE}
library(AppliedPredictiveModeling); library(ggplot2); library(caret);
library(rattle); library(randomForest); library(corrplot);
...
```{r, cache = T}
set.seed(1111) # For reproducibile purpose
